...
Vec vec_rotate2d_ex(Vec vec, float x, float y, double deg);
...
Vec vec_rotate3d(Vec vec, double xdeg,double ydeg, double zdeg){
    ...
    result.data[0] = vec.data[0]*cosx*cosy+vec.data[1]*(cosx*siny*sinz-sinx*cosz)+vec.data[2]*(cosx*siny*cosz+sinx*sinz);
    result.data[1] = vec.data[0]*sinx*cosy+vec.data[1]*(sinx*siny*sinz+cosx*cosz)+vec.data[2]*(sinx*siny*cosz-cosx*sinz);
    result.data[2] = vec.data[0]*(-siny)+vec.data[1]*cosy*sinz+vec.data[2]*cosy*cosz;
...
Vec vec_rotate3d_x(Vec vec, double xdeg){
    ...
    Vec result = vec_alloc(3);
    double xrad = DEG2RAD(xdeg);
    double sinx = VEC_SIN(xdeg);
    double cosx = VEC_COS(xdeg);
...
Vec vec_rotate3d_y(Vec vec, double ydeg){
    ...
    Vec result = vec_alloc(3);
    double yrad = DEG2RAD(ydeg);
    double siny = VEC_SIN(ydeg);
    double cosy = VEC_COS(ydeg);
...
Vec vec_rotate3d_z(Vec vec, double zdeg){
    ...
    Vec result = vec_alloc(3);
    double zrad = DEG2RAD(zdeg);
    double sinz = VEC_SIN(zdeg);
    double cosz = VEC_COS(zdeg);
